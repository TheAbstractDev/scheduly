...
module.exports = {
  ...
  paginate: function (query, data) {
    if (query.offset && query.limit) {
      if (query.limit <= data.length) {
        var paginatedData = []
        for (var i = query.offset; i < query.limit; i++) {
          paginatedData[i] = data[i]
        }
  ...
  getAllJobs: function (req, res) {
    ...
    agenda.jobs((req.params.id) ? {_id: new ObjectId(req.params.id)} : {}, function (err, jobs) {
      ...
      if (req.url === '/' + req._parsedUrl.search) jobsArray.length === 0 ? res.render('index', {title: 'No jobs'}) : res.render('index', {jobs: module.exports.paginate(req.query, jobsArray)})
      ...
      if (req.url === '/webhooks' + req._parsedUrl.search) jobsArray.length === 0 ? res.status(200).json({}) : res.status(200).json(module.exports.paginate(req.query, jobsArray))
    })
  },
