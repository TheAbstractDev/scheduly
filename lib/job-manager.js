  ...
  createJob: function (req, res) {
    if (req.body.url && req.body.scheduling && req.body.body) {
      ...
      try {
        ...
        if (isNaN(humanInterval(req.body.scheduling))) res.status(400).send(req.body.scheduling + ' is not a valid human readable time inteval !')
      }
    } else {
    ...
    if (req.params.id) {
      ...
      agenda.jobs({}, function (err, jobs) {
        if (jobs.length !== 0) {
          ...
          res.status(400).send('Unable to remove jobs')
        }
      })
    }
  },
