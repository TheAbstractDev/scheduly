...
module.exports = {
  ...
  paginate: function (offset, limit, data) {
    limit = (limit <= data.length) ? limit : (data.length < 5 ? data.length : 5)
    offset = offset || 0
    var paginatedData = []
  ...
  getAllJobs: function (req, res) {
    ...
    agenda.jobs((req.params.id) ? {_id: new ObjectId(req.params.id)} : {}, function (err, jobs) {
      ...
      if (req.url === ('/webhooks' + req._parsedUrl.search) || '/webhooks/' + req._parsedUrl.search) jobsArray.length === 0 ? res.status(200).json({}) : res.status(200).json(module.exports.paginate(req.query.offset, req.query.limit, jobsArray))
    })
  },
