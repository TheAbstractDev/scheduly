...
module.exports = {
  ...
  getAllJobs: function (req, res) {
    ...
    agenda.jobs((req.params.id) ? {_id: new ObjectId(req.params.id)} : {}, function (err, jobs) {
      ...
      if (req.url === '/webhooks' + req._parsedUrl.search) jobsArray.length === 0 ? res.status(200).json({}) : res.status(200).json(paginate(req.query.offset, req.query.limit, jobsArray))
      if (req.url === '/webhooks/' + req._parsedUrl.search) jobsArray.length === 0 ? res.status(200).json({}) : res.status(200).json(paginate(req.query.offset, req.query.limit, jobsArray))
    })
  },
