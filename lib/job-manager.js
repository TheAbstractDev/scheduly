...
module.exports = {
  createJob: function (req, res) {
    if (req.body.url && req.body.scheduling && req.body.body) {
      ...
      try {
        ...
        if (isNaN(humanInterval(req.body.scheduling))) res.send(req.body.scheduling + ' is not a valid human readable time distance !')
      }
    } else {
      res.status(400).render('error', {message: 'Missing required parameters'})
    }
  },
  paginate: function (offset, limit, data) {
  ...
  getAllJobs: function (req, res) {
    ...
    agenda.jobs((req.params.id) ? {_id: new ObjectId(req.params.id)} : {}, function (err, jobs) {
      ...
      if (req.url === '/' + req._parsedUrl.search) jobsArray.length === 0 ? res.render('index', {title: 'No jobs'}) : res.render('index', {jobs: module.exports.paginate(req.query.offset, req.query.limit, jobsArray)})
      ...
      if (req.url === '/webhooks' + req._parsedUrl.search) jobsArray.length === 0 ? res.status(200).json({}) : res.status(200).json(module.exports.paginate(req.query.offset, req.query.limit, jobsArray))
      if (req.url === '/webhooks/' + req._parsedUrl.search) jobsArray.length === 0 ? res.status(200).json({}) : res.status(200).json(module.exports.paginate(req.query.offset, req.query.limit, jobsArray))
    })
  },
    ...
    if (req.params.id) {
      agenda.jobs({_id: new ObjectId(req.params.id)}, function (err, jobs) {
        if (jobs.length !== 0) {
          ...
          res.status(400).send('Unable to update the job')
        }
      })
    }
  },
    ...
    if (req.params.id) {
      agenda.jobs({_id: new ObjectId(req.params.id)}, function (err, jobs) {
        if (jobs.length !== 0) {
          ...
          res.status(400).send('Unable to remove the job')
        }
      })
    } else {
      agenda.jobs({}, function (err, jobs) {
        if (jobs.length !== 0) {
          ...
          res.status(400).send('Unable to remove these jobs')
        }
      })
    }
  },
