...
function paginate (previous_cursor, next_cursor, limit, data) {
  ...
  for (var i = 0; i < limit; i++) {
    if (previous_cursor) {
      if (data[i].id.toString().match(previous_cursor)) {
        for (var j = i; j < limit + i; j++) {
          if (!data[j] || data[j] === undefined) break
          paginatedData.push(data[j])
          if (next_cursor && data[j].id.toString().match(next_cursor)) break
        }
      }
    } else {
...
module.exports = {
  ...
  getWebhooks: function (req, res) {
    var jobsArray = []
    if (req.params.id && ObjectId.isValid(req.params.id) === false) {
      res.status(404).send({'error': {
        message: 'Invalid webhooks id',
        status: 404
      }})
    } else {
      agenda.jobs((req.params.id) ? {_id: ObjectId(req.params.id)} : {}, function (err, jobs) {
  ...
  updateWebhooks: function (req, res) {
    if (req.params.id) {
      if (req.params.id && ObjectId.isValid(req.params.id)) {
        agenda.jobs({_id: ObjectId(req.params.id)}, function (err, jobs) {
          if (!err && jobs.length !== 0) {
            if (req.body) {
