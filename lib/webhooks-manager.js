...
module.exports = {
  ...
  getWebhooks: function (req, res) {
    ...
    agenda.jobs((req.params.id) ? {_id: new ObjectId(req.params.id)} : {}, function (err, jobs) {
      ...
      if (req.url === '/' + req._parsedUrl.search) jobsArray.length === 0 ? res.render('index', {title: 'No webhooks'}) : res.render('index', {jobs: paginate(req.query.offset, req.query.limit, jobsArray)})
      ...
      if (req.url === '/webhooks' + req._parsedUrl.search) jobsArray.length === 0 ? res.status(200).send([{}]) : res.status(200).json(paginate(req.query.offset, req.query.limit, jobsArray))
      if (req.url === '/webhooks/' + req._parsedUrl.search) jobsArray.length === 0 ? res.status(200).send([{}]) : res.status(200).json(paginate(req.query.offset, req.query.limit, jobsArray))
    })
  },
